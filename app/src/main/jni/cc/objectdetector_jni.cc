...
JNIEXPORT JNICALL jint TNN_OBJECT_DETECTOR(init)(JNIEnv *env, jobject thiz, jstring modelPath, jint width, jint height, jfloat scoreThreshold, jfloat iouThreshold, jint topk, jint computUnitType)
{
    ...
    protoContent = fdLoadFile(modelPathStr + "/mobilenet_v2.tnnproto");
    modelContent = fdLoadFile(modelPathStr + "/mobilenet_v2.tnnmodel");
    ...
    {
        clsObjectInfo = static_cast<jclass>(env->NewGlobalRef(env->FindClass("com/shuaijun/plant/ObjectInfo")));
